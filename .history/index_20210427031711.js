...
const getCourseAverage = async (studentId) => {
      let [studentMark, data,getStudentCourses ] = [await getStudentMark (studentId), await getData(), await getStudentCourses(studentId) ]
      let testId = studentMark.map(obj => obj.test_id)
      getStudentCourses.forEach(obj => obj.courseAverage)
      // testId.forEach(num)
}
